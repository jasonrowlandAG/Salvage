...
from salvage.engine.privileged import PrivilegedProcess, needs_windows_elevation, run_privileged
...
class PhotoRecEngine:
    ...
    def locate_binary() -> Path | None:
        ...
        if system == "Windows":
            # The Windows binary is named photorec_win.exe, not photorec[.exe], so the
            # shutil.which("photorec") above never matches it even when its folder (e.g.
            # an extracted testdisk-7.2.win64.zip) is on PATH -- check that name too.
            which_win = shutil.which("photorec_win")
            if which_win:
                return Path(which_win)
